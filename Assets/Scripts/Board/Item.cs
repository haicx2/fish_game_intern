...
public class Item
{
    ...
    // NEW: Remembers the original cell so we can return the item later
    ...
    {
        Cell = cell;
        // Keep track of the cell when it's first placed on the board
    ...
    internal virtual void ExplodeView()
    {
        if (View)
        {
            // CHANGED: Now scales completely to 0 instead of 0.1f
            View.DOScale(0f, 0.2f).OnComplete(
                () =>
                {
    ...
    {
        if (View) View.DOPunchScale(View.localScale * 0.1f, 0.1f).SetLoops(-1);
    }
