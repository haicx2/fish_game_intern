...
public class Board
{
    ...
    public bool CheckIfAllCellsAreEmpty()
    {
    ...
    public void EnsureItemsDivisibleByThree()
    {
        List<Cell> filledCells = new List<Cell>();
        foreach (var cell in m_cells)
        {
            if (!cell.IsEmpty) filledCells.Add(cell);
        }
        ...
        // Fill the rest of the board with random sets of 3
        while (typesToSpawn.Count < filledCells.Count)
        {
            NormalItem.eNormalType randomType = Utils.GetRandomNormalType();
            typesToSpawn.Add(randomType);
            typesToSpawn.Add(randomType);
            typesToSpawn.Add(randomType);
        }
        ...
        while (typesToSpawn.Count > filledCells.Count)
        {
            typesToSpawn.RemoveAt(typesToSpawn.Count - 1);
        }

        for (int i = 0; i < typesToSpawn.Count; i++)
        {
            NormalItem.eNormalType temp = typesToSpawn[i];
            int randomIndex = UnityEngine.Random.Range(i, typesToSpawn.Count);
        ...
        for (int i = 0; i < filledCells.Count; i++)
        {
            ...
            filledCells[i].Clear();
            filledCells[i].Assign(item);
            filledCells[i].ApplyItemPosition(false);
        }
    }
}